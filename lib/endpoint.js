...
function SipEndpoint(url, dialogParams, callback) {
    this.url = url;
    this.ws = null;
...
SipEndpoint.prototype.login = function(user, pass) {
    console.log('login not needed');
    return Promise.resolve();
};

SipEndpoint.prototype.attemptInvite = function(sipCall, matrixSide, force) {
    ...
    if (!enoughCandidates && !force) { // don't send the invite just yet
        if (!matrixSide.timer) {
            matrixSide.timer = setTimeout(function() {
                ...
                self.attemptInvite(sipCall, matrixSide, true);
    ...
    // when sent a sip invite including IPv6, and currently the matrix.org
    ...
    return sipCall.invite({
        sdp: matrixSide.offer,
        dialogParams: this.getDialogParamsFor(sipCall, matrixSide),
        sessid: this.sessionId
    });
};

SipEndpoint.prototype.getDialogParamsFor = function(sipCall, callData) {
    var dialogParams = JSON.parse(JSON.stringify(this.dialogParams)); // deep copy
    dialogParams.callID = callData.sipCallId;
    dialogParams.destination_number = sipCall.ext;
    dialogParams.remote_caller_id_number = sipCall.ext;
